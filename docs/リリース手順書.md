...
| `AUTH_SECRET` | Auth.js シークレット | Staging用 | Production用 |
| `AUTH_GOOGLE_ID` | Google OAuth クライアントID | Staging用 | Production用 |
| `AUTH_GOOGLE_SECRET` | Google OAuth クライアントシークレット | Staging用 | Production用 |
| `AUTH_TRUST_HOST` | Hostヘッダを信頼してURL解決 | `true` | `true` |

> **Auth.js運用ルール**:
> - `AUTH_URL` は実アクセスURLと確実に一致する場合のみ設定する。
> - 固定値の使い回しは行わない。
> - 可能な限り `AUTH_TRUST_HOST=true` を利用し、環境差分によるURL不整合を防ぐ。
