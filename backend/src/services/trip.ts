...
export const updateTrip = async (c: Context) => {
  ...
  for (const pn of tripData.plans) {
    const currentPlanData = await transactionDb
      .select({ id: plan.id })
      .from(plan)
    ...
    // 出発地の更新
    const [updatedPlanDepartureLocation] = await transactionDb
      .update(planLocation)
      .set({
    ...
    // 目的地の更新
    const [updatedPlanDestinationLocation] = await transactionDb
      .update(planLocation)
      .set({
    ...
    for (const spot of pn.spots) {
      ...
      const [upsertedPlanSpot] = await transactionDb
        .insert(planSpot)
        .values({
      ...
      if (spot.nearestStation?.placeId && spot.nearestStation.stationType) {
        ...
        await transactionDb
          ...
          .where(eq(planSpotNearestStation.planSpotId, upsertedPlanSpot.id));
      }
    }

    // DBに登録されているが、渡ってきたデータにないスポットを削除する
    if (requestSpotIds.length === 0) {
      await transactionDb.delete(planSpot).where(eq(planSpot.planId, currentPlanData[0].id));
    } else {
      await transactionDb
        .delete(planSpot)
  ...
  // 既に登録されている日付データ
  const existedPlanDate = await transactionDb
    .select({ id: plan.id, date: plan.date })
    .from(plan)
    .where(eq(plan.tripId, tripId));
  // リクエストから飛んできた日付データ
  ...
  // 削除対象の日付のプランデータを削除
  await transactionDb.delete(plan).where(
    and(
  ...
  for (const planData of newPlanData) {
    const [newPlan] = await transactionDb
      .insert(plan)
      .values({
        tripId: tripId,
        date: planData.date,
    ...
    for (const spotData of planData.spots) {
      const [newPlanSpot] = await transactionDb
        .insert(planSpot)
        .values({
          planId: newPlan.id,
          spotId: spotData.id,
    ...
    const [newDeparturePlanLocation] = await transactionDb
      .insert(planLocation)
      .values({
        planId: newPlan.id,
        userId,
    ...
    const [newDestinationPlanLocation] = await transactionDb
      .insert(planLocation)
      .values({
        planId: newPlan.id,
        userId,
    ...
    if (planData.departure.nearestStation?.placeId && planData.departure.nearestStation?.stationType) {
      await transactionDb.insert(planLocationNearestStation).values({
    ...
    if (allNearestStations && allNearestStations.length > 0) {
      for (const stationData of allNearestStations) {
        ...
        await transactionDb.insert(planSpotNearestStation).values({
    ...
    if (planData.departure.transports && createdPlanSpots.length > 0) {
      ...
      await transactionDb.insert(transport).values({
        planId: newPlan.id,
