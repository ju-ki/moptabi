...
export const updateTrip = async (c: Context) => {
  ...
  // planLocationの更新
  tripData.plans.forEach(async (pn) => {
    const currentPlanData = await db
      .select({ id: plan.id })
      .from(plan)
    ...
    // 出発地の更新
    const [updatedPlanDepartureLocation] = await db
      .update(planLocation)
      .set({
    ...
    // DBに登録されているが、渡ってきたデータにない場合は削除する
    await db
      .delete(planLocationNearestStation)
      .where(not(eq(planLocationNearestStation.id, updatedPlanDepartureLocation.id)));

    // 目的地の更新
    const [updatedPlanDestinationLocation] = await db
      .update(planLocation)
      .set({
    ...
    await db
      .delete(planLocationNearestStation)
      .where(not(eq(planLocationNearestStation.id, updatedPlanDestinationLocation.id)));

    // スポットの確認（forEach は async を待てないので for...of に変更）
    for (const spot of pn.spots) {
      // スポットのupsertを行う（upsertした行を取得してそのidを後続処理で使う）
      const [upsertedPlanSpot] = await db
        .insert(planSpot)
        .values({
        ...
        .returning();
      // DBに登録されているが、渡ってきたデータにない場合は削除する
      await db.delete(planSpot).where(
        and(
          eq(planSpot.planId, currentPlanData[0].id),
          not(
            inArray(
              planSpot.spotId,
              pn.spots.map((s) => s.id),
      ...
      // スポットの最寄駅情報を削除（現在のupsertedPlanSpotに紐づかないものを削除）
      await db.delete(planSpotNearestStation).where(not(eq(planSpotNearestStation.planSpotId, upsertedPlanSpot.id)));
    }
  });

  // 既に登録されている日付データ
  const existedPlanDate = await db.select({ id: plan.id, date: plan.date }).from(plan).where(eq(plan.tripId, tripId));
  // リクエストから飛んできた日付データ
  ...
  for (const planData of newPlanData) {
    ...
    const [newDeparturePlanLocation] = await db
      .insert(planLocation)
      .values({
        planId: newPlan.id,
        userId,
    ...
    const [newDestinationPlanLocation] = await db
      .insert(planLocation)
      .values({
        planId: newPlan.id,
        userId,
