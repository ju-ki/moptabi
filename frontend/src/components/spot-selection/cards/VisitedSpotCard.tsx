'use client';
import { Star, MapPin, Check, Link as LinkIcon, Calendar, History } from 'lucide-react';
  ...
  return (
    <div className="w-full">
      <Button
        ...
        type="button"
        variant={'outline'}
        data-testid={`visited-spot-card-${place.id}`}
        ...
        <div className={`flex gap-3 w-full ${isSplitMode ? 'p-2 sm:p-3' : 'p-3 sm:p-4'}`}>
          ...
          <div className="flex-1 min-w-0 flex flex-col justify-between py-0.5">
            ...
            <div className="space-y-2">
              ...
              {place.rating && (
                <div
                  className={`flex items-center gap-1.5 bg-yellow-50 rounded-lg my-4 border border-yellow-200 w-fit ${
                    isSplitMode ? 'px-1.5 py-0.5' : 'px-2 py-1'
                  }`}
                  data-testid={`visited-spot-rating-${place.id}`}
                >
            ...
            {place.address && (
              <div className={`flex items-start gap-1.5 ${isSplitMode ? 'mt-1.5' : 'mt-2'}`}>
                ...
                <p
                  ...
                  data-testid={`visited-spot-address-${place.id}`}
                >
                  {place.address}
                </p>
              </div>
            ...
            {place.url && (
              <div className={`flex items-center gap-1.5 ${isSplitMode ? 'mt-1' : 'mt-1.5'}`}>
                ...
                <a
                  ...
                  data-testid={`visited-spot-url-${place.id}`}
                >
                  外部リンク
                </a>
              </div>
            ...
            {/* 訪問情報（訪問回数 & 前回訪問日時） */}
            <div className={`flex flex-wrap items-center gap-3 ${isSplitMode ? 'mt-1' : 'mt-1.5'}`}>
              {/* 訪問回数 */}
              {place.visitCount !== undefined && place.visitCount > 0 && (
                <div className="flex items-center gap-1">
                  <History size={isSplitMode ? 12 : 14} className="text-purple-500 flex-shrink-0" />
                  <span
                    className={`text-purple-600 font-medium ${isSplitMode ? 'text-[10px]' : 'text-xs'}`}
                    data-testid={`visited-spot-visit-count-${place.id}`}
                  >
              ...
              {place.visitedAt && (
                <div className="flex items-center gap-1">
                  ...
                  <span
                    ...
                    data-testid={`visited-spot-visited-at-${place.id}`}
                  >
                    前回: {formatTimeDate(place.visitedAt)}
                  </span>
                </div>
              )}
            </div>
