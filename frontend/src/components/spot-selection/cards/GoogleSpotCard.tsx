'use client';
import { Star, MapPin, Check, Link as LinkIcon } from 'lucide-react';
...
/**
 * Google検索結果用のスポットカード
 * 表示内容: 画像、スポット名、評価、評価数、住所、外部リンク
 */
interface GoogleSpotCardProps {
  place: Spot;
...
const GoogleSpotCard = ({ place, onSpotClick, isSpotSelected, viewMode = 'list' }: GoogleSpotCardProps) => {
  ...
  return (
    <div className="w-full">
      <Button
        ...
        type="button"
        variant={'outline'}
        data-testid={`google-spot-card-${place.id}`}
            ...
            <div className="space-y-2">
              <div className="flex items-start gap-2 w-full">
                <h4
                  ...
                  data-testid={`google-spot-name-${place.id}`}
                >
                  {place.location.name}
                </h4>
                {isAdded && (
              ...
              {place.rating && (
                <div
                  className={`flex items-center gap-1.5 bg-yellow-50 rounded-lg border border-yellow-200 w-fit ${
                    isSplitMode ? 'px-1.5 py-0.5' : 'px-2 py-1'
                  }`}
                  data-testid={`google-spot-rating-${place.id}`}
                >
                  ...
                  {place.ratingCount !== undefined && (
                    <span
                      className={`text-gray-500 font-medium ${isSplitMode ? 'text-[10px]' : 'text-xs'}`}
                      data-testid={`google-spot-rating-count-${place.id}`}
                    >
            ...
            <div>
              {place.category && place.category.length > 0 && (
                <div
                  ...
                  data-testid={`google-spot-category-${place.id}`}
                >
            ...
            {place.address && (
              <div className={`flex items-start gap-1.5 ${isSplitMode ? 'mt-1.5' : 'mt-2'}`}>
                ...
                <p
                  ...
                  data-testid={`google-spot-address-${place.id}`}
                >
                  {place.address}
                </p>
              </div>
            ...
            {place.url && (
              <div className={`flex items-center gap-1.5 ${isSplitMode ? 'mt-1' : 'mt-1.5'}`}>
                ...
                <a
                  ...
                  data-testid={`google-spot-url-${place.id}`}
                >
                  外部リンク
                </a>
              </div>
